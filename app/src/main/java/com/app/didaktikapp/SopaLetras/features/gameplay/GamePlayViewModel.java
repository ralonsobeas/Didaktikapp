package com.app.didaktikapp.SopaLetras.features.gameplay;
...
import com.app.didaktikapp.SopaLetras.commons.SingleLiveEvent;
import com.app.didaktikapp.SopaLetras.commons.Timer;
import com.app.didaktikapp.SopaLetras.data.GameDataSource;
import com.app.didaktikapp.SopaLetras.data.WordDataSource;
import com.app.didaktikapp.SopaLetras.data.entity.GameDataEntity;
import com.app.didaktikapp.SopaLetras.data.entity.GameDataMapper;
import com.app.didaktikapp.SopaLetras.model.GameData;
import com.app.didaktikapp.SopaLetras.model.UsedWord;
import com.app.didaktikapp.SopaLetras.model.Word;
...
public class GamePlayViewModel extends ViewModel {
    ...
    public void loadGameRound(int gid) {
        if (!(mCurrentState instanceof Generating)) {
            ...
            mGameDataSource.getGameData(gid, new GameDataSource.GameRoundCallback() {
                @Override
                public void onLoaded(GameDataEntity gameRound) {
                    ...
                    if (!mCurrentGameData.isFinished())
                        mTimer.start();
                    GamePlayViewModel.this.setGameState(new Playing(mCurrentGameData));
                }
            });
        }
    }
    ...
    public void generateNewGameRound(final int rowCount, final int colCount) {
        if (!(mCurrentState instanceof Generating)) {
            setGameState(new Generating(rowCount, colCount, "Play me"));

            Observable.create(new ObservableOnSubscribe<GameData>() {
                @Override
                public void subscribe(ObservableEmitter<GameData> emitter) throws Exception {
                    ...
                    .subscribe(new Consumer<GameData>() {
                        @Override
                        public void accept(GameData gameRound) throws Exception {
                            mCurrentDuration = 0;
                            mTimer.start();
